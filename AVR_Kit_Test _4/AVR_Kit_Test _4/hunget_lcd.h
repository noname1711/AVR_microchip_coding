...
{
	lcd4_data_PORT &= 0x0F;					// Xoa bus 4 bit ve 0
	lcd4_control_PORT &= ~(1<<RS);			// RS = 0: du lieu dc gui la cmd
	lcd4_control_PORT |= (1<<E);			// E = 1: chuan bi cho LCD nhan du lieu 
	lcd4_data_PORT |= (lcd4_cmd & 0xF0);	// Xuat 4 bit cao
	lcd4_data_PORT |= (lcd4_cmd & 0xF0);
	lcd4_control_PORT &= ~(1<<E);			// E tu 1 xuong 0: doc du lieu tu data bus 
